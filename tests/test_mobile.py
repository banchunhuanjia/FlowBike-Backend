...
def run_test():
    ...
    # 【改动2】构造 mobile_request
    req = bike_pb2.mobile_request()
    req.mobile = "13812345678"  # 只需要手机号
    ...
    try:
        ...
        if response.status_code == 200:
            # 【改动3】解析 mobile_response
            resp_pb = bike_pb2.mobile_response()
            try:
                resp_pb.ParseFromString(response.content)
                print("[+] 解析响应成功!")
                print(f"    Code (状态码): {resp_pb.code}")
                print(f"    ICode (验证码): {resp_pb.icode}")  # 这里应该就是服务器生成的验证码
                print(f"    Data: {resp_pb.data}")
